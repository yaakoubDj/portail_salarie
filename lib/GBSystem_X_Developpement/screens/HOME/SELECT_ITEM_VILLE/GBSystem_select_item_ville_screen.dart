...
import 'package:portail_salarie/GBSystem_X_Developpement/screens/HOME/SELECT_ITEM_VILLE/GBSystem_select_item_ville_controller.dart';
...
class GBSystem_SelectItemVilleScreen extends StatelessWidget {
  const GBSystem_SelectItemVilleScreen(
      {super.key, required this.controllerTxtField});

  final TextEditingController controllerTxtField;
  @override
  Widget build(BuildContext context) {
    final GBSystemSelectItemVilleController m =
        Get.put(GBSystemSelectItemVilleController(context: context));
              ...
              body: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    m.text != null && m.text != ''
                        ? m.filtredVilles.isNotEmpty
                            ? Expanded(
                                child: ListView.builder(
                                  itemCount: m.filtredVilles.length,
                                  itemBuilder: (context, index) {
                                    return GBSystem_Root_CardView_Ville_Widget(
                                      tileColor: index % 2 == 0
                                      ...
                                      onTap: () {
                                        m.selectItemSiteFunction(
                                            selectedSite:
                                                m.filtredVilles[index]);

                                        controllerTxtField.text =
                                            m.filtredVilles[index].VIL_LIB;
                                      },
                                      ville: m.filtredVilles[index],
                                    );
                                  },
                                ),
                              )
                                ...
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    return GBSystem_Root_CardView_Ville_Widget(
                                      ...
                                      onTap: () {
                                        m.selectItemSiteFunction(
                                            selectedSite: m.villes[index]);
                                        controllerTxtField.text =
                                            m.villes[index].VIL_LIB;

                                        // m.sitePlanningController
                                        //     .setCurrentSite = m.villes[index];
                                        // Get.back();
                                      },
                                      ville: m.villes[index],
                                    );
                                  },
                                ),
                              )
