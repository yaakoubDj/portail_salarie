    ...
    return Container(
      ...
      child: StaggeredGridView.countBuilder(
        ...
        staggeredTileBuilder: (index) {
          return StaggeredTile.fit(1); // Fit one column
        },
        itemBuilder: (context, index) {
          final events = controller.getEventsOnDay(monthDays[index]);
          // Determine the row index
          int rowIndex = index ~/ 7; // 7 is the number of columns
          double maxHeight = 0;

          // Find the maximum height for the current row
          for (int i = 0; i < 7; i++) {
            if ((rowIndex * 7) + i < 42) {
              // Ensure index is within bounds
              final rowEvents =
                  controller.getEventsOnDay(monthDays[(rowIndex * 7) + i]);
              //  final rowEventCount = rowEvents.length;
              int rowEventCount = 0; //rowEvents.length;

              for (int j = 0; j < rowEvents.length; j++) {
                rowEventCount =
                    rowEventCount + rowEvents[j].title.split('\n').length;
              }
              ;
              double rowCellHeight = 40.0 + (rowEventCount * 20.0);
              if (rowCellHeight > maxHeight) {
                maxHeight = rowCellHeight;
              }
            }
          }
          return GestureDetector(
            ...
            child: Container(
              height:
                  maxHeight, // Set the height to the maximum height for the row
